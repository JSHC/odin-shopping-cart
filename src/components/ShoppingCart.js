...
const ShoppingCart = (props) => {
    ...
    return (
        <div className="shopping-cart">
            {showPurchaseConfirm && (
                <PurchaseConfirmation onClose={onPurchaseConfirmClose} />
            )}
            <h1>Shopping Cart</h1>
                ...
                {cartItems.map((item) => {
                    return (
                        <div className="shopping-cart-item" key={item.id}>
                            <p className="shopping-cart-item-name">
                                {item.name}
                            </p>
                            <p className="shopping-cart-item-quantity">
                                {item.quantity}
                            </p>
                ...
                {(cartItems && cartItems.length) > 0 ? (
                    <div className="shopping-cart-bottom">
                        ...
                        <p className="shopping-cart-sum">
                            Total amount:{" "}
                            <span className="shopping-cart-sum-amount">
                                {cartItems.reduce(
                                    (prev, curr) =>
                    ...
                    <p className="shopping-cart-empty-message">
                        You haven't added any items to the shopping cart.
                    </p>
                )}
            </div>
        </div>
